...
def test_valid_request_to_download_youtube_video_from_txt_file(
    fastapi_testing_client: TestClient,
    capsys: pytest.CaptureFixture,
) -> None:
    response = fastapi_testing_client.post(
        "/api/v1/download/video/youtube/from_txt_file_to_root_dir",
    ...
    assert (
        f"[x] Published message to topic [{DownloadYouTubeVideoFromTxtFileCommand.get_topic()}] ..."
        in captured.out
    )


def test_invalid_request_to_download_youtube_video_from_txt_file(
    fastapi_testing_client: TestClient,
) -> None:
    response = fastapi_testing_client.post(
        "/api/v1/download/video/youtube/from_txt_file_to_root_dir",
...
def test_valid_request_to_download_youtube_video_from_txt_file_with_invalid_rabbitmq_connection(
