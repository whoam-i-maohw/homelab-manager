...
def test_valid_request_to_download_youtube_video_from_txt_file_to_channel_name_dir(
    fastapi_testing_client: TestClient,
    capsys: pytest.CaptureFixture,
) -> None:
    response = fastapi_testing_client.post(
        "/api/v1/download/video/youtube/from_txt_file_to_channel_name_dir",
        json=DownloadVideoRequest.FromTxtFile(
            txt_file_path="any-path",
    ...
    assert (
        f"[x] Published message to topic [{DownloadYouTubeVideoFromTxtFileToChannelNameDirCommand.get_topic()}] ..."
        in captured.out
    )


def test_invalid_request_to_download_youtube_video_from_txt_file_to_channel_name_dir(
    fastapi_testing_client: TestClient,
) -> None:
    response = fastapi_testing_client.post(
        "/api/v1/download/video/youtube/from_txt_file_to_channel_name_dir",
        json=dict(desired_download_path="any-path"),
    )
...
def test_valid_request_to_download_youtube_video_from_txt_file_to_channel_name_dir_with_invalid_rabbitmq_connection(
